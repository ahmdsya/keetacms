...
@section('tittle', __('Pengaturan Komentar - KeetaCMS Admin'))
@section('konten')
  ...
  <section class="content">
    ...
    <div class="box">
        <div class="box-header with-border">
          <h3 class="box-title">{{ __('Pengaturan Komentar')}}</h3>
        	...
        	<div style="margin-left: 50px;margin-right: 50px;">         	
          	<table class="table table-striped">
          		...
          		<tbody>
          			...
          			<tr>
          				<th>{{$setting->key}}</th>
          				@if($setting->key == 'show_comment')
          				<td>{{($setting->value == 'Y' ? 'YES' : 'NO')}}</td>
          				@else
          				<td>{{($setting->value)}}</td>
          				@endif
          				<td>
          			...
          			<div class="modal fade" id="modal-{{$setting->id}}">
	                    <div class="modal-dialog">
	                      <div class="modal-content">
	                        ...
	                        <form method="POST" action="{{route('comment.update', $setting->id)}}">
	                            @csrf
	                            @method('PUT')
	                        	...
	                        	<div class="form-group">
		                          <label for="value">{{ __('VALUE')}}</label>
		                          	...
		                          	<select class="form-control" name="value" id="value">
		                          		<option value="Y" {{($setting->value == 'Y' ? 'selected' : '')}}>YES</option>
		                          		<option value="N" {{($setting->value == 'N' ? 'selected' : '')}}>NO</option>
		                          	</select>
		                          @else
